...
{
	// My solution: sum of four numbers can be divided to sum of
	// the two sums of two pairs
	public List<List<Integer>> fourSum(int[] nums, int target)
	{
		// Each element has a pair of numbers and their sum
		// total number of pairs: (1 + (n - 1)) * (n - 1) / 2
		...
		for (int i = 0; i < index - 1; i++)
		{
			for (int j = i + 1; j < index; j++)
			{
				...
				{
					results.add(Arrays.asList(nums[pairs[i][0]], nums[pairs[i][1]],
											  nums[pairs[j][0]], nums[pairs[j][1]]));
				}
			}
		}
